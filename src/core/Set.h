...
namespace nspace{
  ...
  public:
    ...
    // sorts the elements of this set by using the compare function
    void sort(std::function<bool (const T, const T) > compare);
    // sorts the elements of this set by comparing their int values (obtained by the toIntValue function)
    ...
    // tries to add item by dynamically casting it to T
    template<typename T2> bool tryAdd(T2 item);
    // tries to remove item by dynamically casting it to T
