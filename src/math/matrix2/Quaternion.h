...
namespace matrix2{
  ...
  public:
    ...
    static inline void fromAxisAngle(Quat & q, const AxisVectorType & axis, const T & angle){
      T a = 0.5 * angle;
      T sinus;
      ScalarOperations<T>::cos(q.w(),a);
      ScalarOperations<T>::sin(sinus,a);
      q.x() = sinus * axis(0);
      q.y() = sinus * axis(1);
      q.z() = sinus * axis(2);
    }
  ...
  public:
    ...
    inline StaticMatrix<T,3,3> rotationMatrix()const{
    ...
    inline StaticMatrix<T,3,3> transposedRotationMatrix()const{
      StaticMatrix<T,3,3> R;
