...
namespace nspace{
  ...
  struct TypedCallableInfo : public CallableInfo{
    ...
    TypedCallableInfo(TCallable info):_info(info){
      ToArgumentInfo toInfo = [this](std::shared_ptr<ArgumentInfo> info){
        Arguments().push_back(info);
      };
      callable_traits<TCallable>::arguments::for_each(toInfo);
    ...
    Argument callImplementation(const ArgumentList & args)const{
      return call_callable(_info,args);
    }
  };
